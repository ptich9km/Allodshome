...
> Последнее обновление: 24.09 — минимальная алхимия: Druid Shop, 2 рецепта и контейнерная панель по `rpg` / `godot-ui-control` / `game-ui-ux` / `godot-gdscript`. Требуется ручная визуальная проверка.
...
### Генерация карт — новый пайплайн (в работе: этап `_place_terrain`→`_place_cities`→`_connect_cities`→`_place_mountains_water`→`_place_city_content`→`_place_objects`→`_place_herbs`→`_place_greys` готов и проверен)
...
- **Файлы:** `tests/gen_smart_map.gd` — этапы + sidecar-ы `structures/npcs/herbs`; `scripts/herb_node.gd` — сбор с регенерацией 180 секунд; `assets/professions/alchemy/recipes.json` + `scripts/alchemy_panel.gd` — минимальная алхимия.
- **Осталось:** расширение рецептов и эффектов зелий, шумные профили городов, арт городов, механика кузницы (переплавка разбитой брони), фракции городов по зонам.
