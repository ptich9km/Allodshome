...
> Последнее обновление: 24.09 — прозрачность herb-ассетов и равномерный посев по карте по `create-game-assets` / `procedural-gen` / `godot-gdscript`. Требуется ручная визуальная проверка.
...
- **Файлы:** `tests/gen_smart_map.gd` — этапы + sidecar-ы `structures/npcs/herbs`; `scripts/herb_node.gd` — сбор с регенерацией 180 секунд.
- **Осталось:** рецепты и панель алхимии, шумные профили городов, арт городов, механика кузницы (переплавка разбитой брони), фракции городов по зонам.
