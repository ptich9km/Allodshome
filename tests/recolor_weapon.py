#!/usr/bin/env python3
"""Recolor metal sword to create 19 material variants."""

from PIL import Image
import colorsys

# Base sword image
BASE_IMAGE = "/media/alexey/EEF4D0F9F4D0C4CD/Work/Allodshome/import/db0206c9-c389-4122-a9f2-a325649ae532.png"
...
def recolor_image(img, target_rgb):
    ...
    for y in range(height):
        for x in range(width):
            ...
            # Check if pixel is background (near-white)
...
def main():
    ...
    for mat_name, color_rgb in MATERIALS.items():
        ...
        path = f"{DST}/{mat_name}_weapon.png"
        variant.save(path)
        print(f"  {mat_name}_weapon.png → RGB{color_rgb}")

    print(f"\nDone! {len(MATERIALS)} weapon variants saved to {DST}")


if __name__ == "__main__":
    print("Recoloring sword to 19 materials...")
    main()
